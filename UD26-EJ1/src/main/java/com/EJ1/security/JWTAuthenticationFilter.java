package com.EJ1.security;


import static com.EJ1.security.Constants.*;
...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		String token = Jwts.builder()	//creacion token
				.setIssuedAt(new Date()) //fecha
				.setIssuer(ISSUER_INFO) 
				.setSubject(user.getUsername()) //usuario
				.setExpiration(new Date(System.currentTimeMillis() + TOKEN_EXPIRATION_TIME)) //expiracion
				.claim("roles", authorities) //roles
				.signWith(SignatureAlgorithm.HS512, SUPER_SECRET_KEY).compact(); 
		
		//token con cabecera bearer
